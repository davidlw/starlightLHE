...
void anaSTARlight(TString parSpec = "slight")
{
    TH1::SetDefaultSumw2(kTRUE);

    TH1D *hnEvts        = new TH1D("hnEvts", "hnEvts", 5, -0.5, 4.5);
    TH3D *hMvsPtvsRap_Raw    = new TH3D("hMvsPtvsRap_Raw", "hMvsPtvsRap_Raw; y; p_{T} (GeV); M_{KK} (GeV); Entries", 50, -2.5, 2.5, 1000, 0, 2, 300, 0.5, 1.5);
    TH3D *hMvsPtvsRap_RapSel = new TH3D("hMvsPtvsRap_RapSel", "hMvsPtvsRap_RapSel; y; p_{T} (GeV); M_{KK} (GeV); Entries", 50, -2.5, 2.5, 1000, 0, 2, 300, 0.5, 1.5);
    TH3D *hMvsPtvsRap      = new TH3D("hMvsPtvsRap", "hMvsPtvsRap; y; p_{T} (GeV); M_{KK} (GeV); Entries", 50, -2.5, 2.5, 1000, 0, 2, 300, 0.5, 1.5);
    ...
    TH3D *hRapvsNegPtvsPosPt    = new TH3D("hRapvsNegPtvsPosPt", "hRapvsNegPtvsPosPt; K^{+} p_{T} (GeV); K^{-} p_{T} (GeV)", 500, 0, 5, 500, 0, 5, 50, -2.5, 2.5);
    TH3D *hRapvsNegEtavsPosEta  = new TH3D("hRapvsNegEtavsPosEta", "hRapvsNegEtavsPosEta; K^{+} #eta; K^{-} #eta", 100, -2.5, 2.5, 100, -2.5, 2.5, 50, -2.5, 2.5);
    TH2D *hNegPtvsPosPt    = new TH2D("hNegPtvsPosPt", "hNegPtvsPosPt; K^{+} p_{T} (GeV); K^{-} p_{T} (GeV)", 500, 0, 5, 500, 0, 5);
    TH2D *hNegEtavsPosEta  = new TH2D("hNegEtavsPosEta", "hNegEtavsPosEta; K^{+} #eta; K^{-} #eta", 100, -2.5, 2.5, 100, -2.5, 2.5);
    TH2D *hNegPhivsPosPhi  = new TH2D("hNegPhivsPosPhi", "hNegPhivsPosPhi; K^{+} #phi; K^{-} #phi", 120, -PI, PI, 120, -PI, PI);

    TH2D *hPtvsRap_Raw = new TH2D("hPtvsRap_Raw", "hPtvsRap_Raw; y; p_{T} (GeV); Entries", 50, -2.5, 2.5, 1000, 0, 2);
    TH2D *hPtvsEta_Dau = new TH2D("hPtvsEta_Dau", "hPtvsEta_Raw; #eta; p_{T} (GeV); Entries", 50, -2.5, 2.5, 1000, 0, 2);
    ...
    while (getline(infile,temp_string)) {
        ...
        //    cout<< px << "   " << py << "   " << pz << "   " << pdg_id << endl;
            ...
            if(TMath::Abs(pdg_id) != 321) continue;
        ...
        if(nPosMu==1 && nNegMu==1){
            ...
            hPtvsRap_Raw->Fill(motherPar.Rapidity(), motherPar.Pt());

            if(TMath::Abs(y)>2.4) continue;

            hPtvsEta_Dau->Fill(posParMom.Eta(), posParMom.Pt());
            hPtvsEta_Dau->Fill(negParMom.Eta(), negParMom.Pt());
            ...
            hRapvsNegPtvsPosPt->Fill(posParMom.Pt(), negParMom.Pt(), motherPar.Rapidity());
            hRapvsNegEtavsPosEta->Fill(posParMom.Eta(), negParMom.Eta(), motherPar.Rapidity());

//	    cout<<posParMom.Eta()<<negParMom.Eta()<<motherPar.Rapidity()<<endl;
...
*/
    ...
    hRapvsNegPtvsPosPt->Write();
    hRapvsNegEtavsPosEta->Write();    
    fOut->Close();
}
